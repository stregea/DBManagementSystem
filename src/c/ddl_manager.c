                    ...
                    for (int i = 0; i < table_to_alter->attribute_count; i++) {
                        if (strcasecmp(table_to_alter->attributes[i]->name, tokenizer) == 0) {
                            ...
                            for (int j = 0; j < table_to_alter->primary_key->size; j++) {
                                printf("comparing %s to %s\n", table_to_alter->attributes[i]->name,
                                       table_to_alter->primary_key->attributes[j]->name);
                                if (strcmp(table_to_alter->primary_key->attributes[j]->name,
                                           table_to_alter->attributes[i]->name) == 0) {
...
//                if(which_token == 1){
//                    tokenizer = strtok_r(NULL, " ,)", token);
//                    // once comma read tokenizer = null
//
//                }else{
//                    tokenizer = strtok_r(NULL, " ,)", &temp_token);
//                }
...
//                    tokenizer = strtok_r(NULL, " (),", token);
    ...
    if (attribute_names != NULL && referenced_table_name != NULL && referenced_table_attributes != NULL) {
        printf("attr names: %s, referenced table: %s, referenced attrs: %s\n", attribute_names, referenced_table_name,
               referenced_table_attributes);
...
int char_or_varchar(char*word){
    ...
    // varchar
    else if((word[0] == 'v' || word[0] == 'V') && (word[1] == 'a' || word[1] == 'A') && (word[2] == 'r' || word[1] == 'R')
    && (word[3] == 'c' || word[3] =='C') && (word[4] == 'h' || word[4] == 'H') && (word[5] == 'a' || word[5] == 'A')
        && (word[6] == 'r' || word[6] == 'R')){
        return 0;
    }
    return -1;
